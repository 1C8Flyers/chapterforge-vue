  ...
  const normalizeCustomFormsConfig = (input = []) => {
    ...
    rawList.forEach((form) => {
      ...
      normalized.push({
        ...
        slug,
        enabled: Boolean(merged.enabled),
        addToParticipation: Boolean(merged.addToParticipation),
...
app.get('/api/members/:id/participation', async (req, res) => {
  try {
    ...
    const customForms = await getCustomFormsConfig();
    const participationForms = new Set(
      customForms.filter(form => form.addToParticipation).map(form => form.slug)
    );
    if (participationForms.size > 0) {
      const customSignups = await db.listCustomFormSignupsByMemberId(req.params.id);
      customSignups
        .filter(signup => participationForms.has(signup.FormKey))
        .forEach(signup => {
          items.push({
            type: signup.FormName || 'Custom Form',
            formKey: signup.FormKey,
